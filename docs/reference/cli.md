...
The detector waits for the pre-trigger history to fill, then requires a return beyond
the tee to advance across frames after approach motion. A reversal alone does not
trigger. Approach state expires after more than two missing frames or 60 ms without
confirmation. These checks reduce false triggers but do not prove ball identity;
validate tee geometry and thresholds with real shots. The default capture remains
72 ms at 3 ms/frame, and selected samples retain IQ16 precision. Full capture is a
diagnostic alternative, not a longer recording window.

The revised detector requires firmware built from the matching source. The candidate
image is `firmware/releases/l3_dump_self_trigger_20260925.bin`; it requires hardware
validation before range use.
